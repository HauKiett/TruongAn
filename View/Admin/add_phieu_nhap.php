...
<div class="container mt-4">
    <h3 class="text-center mb-4">Thêm phiếu nhập kho</h3>
    <form method="POST" action="">
        <div class="mb-3">
            <label for="ncc" class="form-label">Nhà cung cấp</label>
            <input type="text" name="nhacungcap" class="form-control" id="ncc" required>
        </div>
        <div class="mb-3">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $stmt = $conn->prepare("INSERT INTO phieunhap (nhacungcap, diachi, lydo, ngay) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $_POST['nhacungcap'], $_POST['diachi'], $_POST['lydo'], $_POST['ngay']);
    $stmt->execute();
    ...
    for ($i = 0; $i < count($_POST['tensp']); $i++) {
        ...
        $stmt2 = $conn->prepare("INSERT INTO chitietphieunhap (id_phieu, tensp, soluong, dongia) VALUES (?, ?, ?, ?)");
    ...
    echo "<script>location='export_pdf_nhap.php?id=$id_phieu'</script>";
}
?>
